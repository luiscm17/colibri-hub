# Product Requirements

Product Requirements Documents (PRDs) define business rules, acceptance criteria, and capability boundaries. Each PRD is the single normative source for its capability.

## Product Overview

- [Product Overview](product-overview.md)
- [UI Requirements](ui-requirements.md)

## Warehouse

- [Warehouse Overview](warehouse/overview.md)
- [Bale Management](warehouse/bale-management.md)
- [Finished Product](warehouse/finished-product.md)
- [Production Supplies](warehouse/production-supplies.md)

## Operation

- [Operation Overview](operation/overview.md)

## Access Control

- [Access Control](access-control.md)
