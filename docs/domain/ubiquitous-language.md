...
| `en Producción`                         | Bale has been delivered and is under Production custody                                 | **in Production**                         | `IN_PRODUCTION`                      | Does not mean consumed or processed.                                                                                            |
| `fecha de entrega`                      | Time when the bale delivery fact occurred                                                | **delivered at**                          | `delivered_at`                       | Current mandatory delivery evidence; delivery actors are not currently mandatory.                                             |
...
- `Bale` has independent technical identity and owns the `IN_WAREHOUSE` to `IN_PRODUCTION` lifecycle; its business-visible identity is `shipment_number` + `bale_number`.
