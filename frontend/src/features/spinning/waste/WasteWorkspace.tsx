...
export function WasteWorkspace({ gateway = developmentSpinningGateway }: { gateway?: SpinningGateway }) {
  ...
  return <Stack gap="lg">
    ...
    <DataGridShell
      ...
      statusBar={<Alert role="status" aria-live="polite" color="blue">{references ? 'Registre únicamente el peso real pesado. Las madejas fuera de especificación se reprocesan y no son desperdicio.' : 'Las referencias de desperdicio no están disponibles hasta que el servicio esté disponible.'}</Alert>}
