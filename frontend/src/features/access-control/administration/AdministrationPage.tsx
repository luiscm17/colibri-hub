...
export default function AdministrationPage({ family: declaredFamily, mode }: { family?: string; mode?: 'edit' } = {}) {
  ...
  }, [filterKey, operation?.endpoint, operation?.family, operation?.request, page, reload, requestKey, subjectId])
  ...
  if (operation.renderer === 'preset-create') return <AdministrationShell route={{ family: operation.family, criteria, page, subjectId, mode }} origin={origin} navigate={projectRoute}>
    {({ setDraftState, requestDeparture }) => <Stack gap="lg"><Group justify="space-between"><Title order={1}>Create preset</Title><Button variant="subtle" onClick={() => requestDeparture()}>Cancel</Button></Group><PresetWorkflow onDirtyChange={(dirty) => setDraftState('new preset', dirty)} /></Stack>}
  </AdministrationShell>
