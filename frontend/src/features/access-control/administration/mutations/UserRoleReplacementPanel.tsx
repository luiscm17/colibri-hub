...
function UserRoleReplacementContent({ userId, version, roleIds, authorityGeneration }: { userId: string; version: number; roleIds: string[]; authorityGeneration: string }) {
  ...
  return <Stack mt="md"><Text fw={500}>Replace user roles</Text><TextInput label="Role IDs" value={draft} onChange={(event) => { gate.invalidate(); setConfirming(false); setPreview(null); setDraft(event.currentTarget.value) }} description="Comma-separated role IDs. Changes require a fresh preview." /><Group><Button onClick={() => void previewChange()}>Preview replacement</Button>{preview ? <Button color="red" onClick={() => setConfirming(true)}>Review replacement</Button> : null}</Group>{preview ? <Alert>Users affected by this proposed change: {preview.affectedUserCount}.</Alert> : null}{message ? <Alert>{message}</Alert> : null}<Modal opened={confirming} onClose={() => setConfirming(false)} title="Confirm replacement" closeOnClickOutside={false} returnFocus><Stack><Text>Replace user roles</Text><Text role="status" aria-live="polite">Users affected by this proposed change: {preview?.affectedUserCount ?? 0}.</Text><Text>Confirming applies this preview once. A new preview is required after any conflict or edit.</Text><Group justify="flex-end"><Button variant="default" onClick={() => setConfirming(false)}>Cancel</Button><Button color="red" onClick={() => void applyChange()}>Confirm replacement</Button></Group></Stack></Modal></Stack>
}
