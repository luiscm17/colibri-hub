...
function UserRoleReplacementContent({ userId, version, roleIds, authorityGeneration }: { userId: string; version: number; roleIds: string[]; authorityGeneration: string }) {
  ...
  async function previewChange() { const request = gate.previewRequest(draft.split(',')); const generation = gate.currentRequestGeneration(); if (!request) { setPreview(null); setMessage('Choose a different role set before previewing.'); return }; try { const result = await httpJson<{ subject_version: number; affected_user_count: number; affected_users: AffectedUser[] }>(request.path, { method: request.method, body: request.body, recoverAccessDenied: true }); if (gate.acceptPreview({ subjectVersion: result.subject_version, affectedUserCount: result.affected_user_count }, draft.split(','), generation)) setPreview({ subjectVersion: result.subject_version, affectedUserCount: result.affected_user_count, affectedUsers: result.affected_users }) } catch { gate.invalidate(); setPreview(null); setMessage('Preview is no longer current. Review the roles and preview again.') } }
  async function applyChange() { const request = gate.applyRequest(); if (!request) return; try { await httpJson(request.path, { method: request.method, body: request.body, recoverAccessDenied: true }); setConfirming(false); setMessage('User roles replaced.') } catch { gate.invalidate(); setConfirming(false); setPreview(null); setMessage('The replacement was not applied. Preview a fresh change before retrying.') } }
  return <Stack mt="md"><Text fw={500}>Replace user roles</Text><TextInput label="Role IDs" value={draft} onChange={(event) => { gate.invalidate(); setConfirming(false); setPreview(null); setDraft(event.currentTarget.value) }} description="Comma-separated role IDs. Changes require a fresh preview." /><Group><Button onClick={() => void previewChange()}>Preview replacement</Button>{preview ? <Button color="red" onClick={() => setConfirming(true)}>Review replacement</Button> : null}</Group>{preview ? <Alert><ImpactPreview affectedUserCount={preview.affectedUserCount} affectedUsers={preview.affectedUsers} /></Alert> : null}{message ? <Alert>{message}</Alert> : null}<Modal opened={confirming} onClose={() => setConfirming(false)} title="Confirm replacement" closeOnClickOutside={false} returnFocus><Stack><Text>Replace user roles</Text><Text role="status" aria-live="polite">Users affected by this proposed change: {preview?.affectedUserCount ?? 0}.</Text><Text>Confirming applies this preview once. A new preview is required after any conflict or edit.</Text><Group justify="flex-end"><Button variant="default" onClick={() => setConfirming(false)}>Cancel</Button><Button color="red" onClick={() => void applyChange()}>Confirm replacement</Button></Group></Stack></Modal></Stack>
}
