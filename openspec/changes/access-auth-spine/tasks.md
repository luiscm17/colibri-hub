...
| 2 | Start from PR #1; finish SQLAlchemy, migration, safeguards, audit persistence, and PostgreSQL proof. | Suggested `back/access-auth-spine-persistence`, PR #2 targets PR #1 branch. | Focused persistence tests, then guarded integration suite. | User runs `pnpm supabase db reset --local --no-seed`; then `TEST_DATABASE_URL=... uv run --locked --package backend python -m unittest discover -s backend/integration_tests -v`. | Revert Access persistence, registry, migration, and integration tests; retain abstract core. |
| 3 | Start from PR #2; finish HTTP/composition, `/me`, CORS, and only the bale write gate. | Suggested `back/access-auth-spine-http`, PR #3 targets PR #2 branch; tracker ultimately targets `main`. | Focused API/runtime tests, then backend unit suite. | Inject resolver/port into `create_app`; no installation or production identity provider. | Revert HTTP/bootstrap/Warehouse adapter changes and API tests; retain PR #1–2. |
...
- [ ] 3.1 Create `backend/src/warehouse/bales/ports/authorization.py` with opaque `AuthenticatedIdentity` and narrow consumer-owned `AuthorizationPort`; Access must not own this contract.
- [ ] 3.2 Create `backend/src/access/adapters/warehouse_authorization.py` and `/api/v1/access/me`; test ordinary/global snapshots and specific missing/inactive self outcomes versus generic business denial.
- [ ] 3.3 Modify `backend/src/bootstrap/{http_application,api_router,warehouse_bale_dependency}.py` for composition, deterministic test identity injection, fail-closed `401`, route discovery, and `Authorization` CORS header.
- [ ] 3.4 Modify `backend/src/warehouse/bales/adapters/http/router.py` so only `POST /api/v1/warehouse/bales` derives `write + warehouse.raw_materials` and authorizes before mapping/validation/mutation; test allowed, denied, client-authority-ignored, and unaffected endpoints.

## Phase 4: Chain Verification

- [ ] 4.1 Verify each child diff against its immediate parent, record focused/runtime evidence, preserve the user-only installation rule, and do not create branches, commits, PRs, or invoke review lifecycle commands.
