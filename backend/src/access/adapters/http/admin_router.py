...
def create_admin_router(
    ...
    def replace_user_roles(
        ...
        use_cases.replace_user_roles.execute(
            ReplaceUserRolesCommand(
                user_id=str(user_id),
    ...
    def preview_user_role_replacement(
        ...
        return _preview_response(
            use_cases.preview_user_role_replacement.execute(
                user_id=str(user_id), role_ids=body.role_ids
            )
        )
    ...
    def get_user(
        ...
        result = use_cases.get_access_user.execute(user_id=str(user_id))
    ...
    def change_user_status(
        ...
        user = use_cases.user_repository.find_by_id(str(user_id))
        if user is None:
            from fastapi import HTTPException, status

            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="access_user_not_found"
            )
    ...
    def create_role(
        ...
        result = use_cases.create_role.execute(
            CreateRoleCommand(
                role_code=body.role_code,
                role_name=body.role_name,
                description=body.description,
                permissions=[
                    DtoPermissionInput(action=p.action, scope_id=p.scope_id)
    ...
    def get_role(
        ...
        role = use_cases.role_repository.find_by_id(str(role_id))
        if role is None:
            from fastapi import HTTPException, status

            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="access_role_not_found"
            )
        return RoleResponse(
    ...
    def update_role(
        ...
        result = use_cases.update_role.execute(
            UpdateRoleCommand(
                role_id=str(role_id),
                role_name=body.role_name,
                description=body.description,
    ...
    def preview_role_change(
        ...
        return _preview_response(
            use_cases.preview_role_change.execute(
                role_id=str(role_id), permissions=permissions
            )
        )
    ...
    def list_role_presets(
        ...
        page: int = Query(default=1, ge=1),
        page_size: int = Query(default=50, ge=1, le=100),
    ) -> PaginatedResponse[RolePresetResponse]:
        result = use_cases.list_role_presets.execute(page=page, page_size=page_size)
        return PaginatedResponse(
            items=[preset_response(p) for p in result.items],
            page=page,
            page_size=page_size,
            total=result.total,
        )
    ...
    def create_role_preset(
        identity: Annotated[AuthenticatedIdentity, Depends(_require_admin)],
        use_cases: Annotated[AdminUseCases, Depends(admin_use_case_provider)],
        body: CreateRolePresetRequest,
    ) -> RolePresetResponse:
        return preset_response(
            use_cases.create_role_preset.execute(
                CreateRolePresetCommand(
                    body.preset_code,
                    body.preset_name,
                    body.description,
                    [
                        DtoPermissionInput(p.action, p.scope_id)
                        for p in body.permissions
                    ],
                    body.reason,
                    _resolve_user_id(use_cases, identity.subject),
                    use_cases.identity.generate_operation_id(),
                )
            )
        )
    ...
    def get_role_preset(
        ...
        return preset_response(
            use_cases.get_role_preset.execute(preset_id=str(preset_id))
        )
    ...
    def update_role_preset(
        ...
        identity: Annotated[AuthenticatedIdentity, Depends(_require_admin)],
        use_cases: Annotated[AdminUseCases, Depends(admin_use_case_provider)],
        body: UpdateRolePresetRequest,
    ) -> RolePresetResponse:
        return preset_response(
            use_cases.update_role_preset.execute(
                UpdateRolePresetCommand(
                    str(preset_id),
                    body.preset_name,
                    body.description,
    ...
    def create_role_from_preset(
        preset_id: UUID,
        identity: Annotated[AuthenticatedIdentity, Depends(_require_admin)],
        use_cases: Annotated[AdminUseCases, Depends(admin_use_case_provider)],
        body: CreateRoleFromPresetRequest,
    ) -> RoleResponse:
        result = use_cases.create_role_from_preset.execute(
            CreateRoleFromPresetCommand(
                str(preset_id),
                body.role_code,
                body.role_name,
    ...
    def list_scope_definitions(
        ...
        return [
            ScopeDefinitionResponse(
                ...
                supported_actions=d.supported_actions,
    ...
    def register_scope(
        ...
        result = use_cases.register_recognized_scope.execute(
            RegisterRecognizedScopeCommand(
        ...
        return ScopeResponse(
            ...
            owning_context=result.owning_context,
            is_active=result.is_active,
            version=result.version,
        )
...
def _preview_response(result) -> ImpactPreviewResponse:
    return ImpactPreviewResponse(
        ...
        affected_users=[
            ...
            for item in result.affected_users
        ],
        permissions_added=[
            PermissionResponse(action=item.action, scope_code=item.scope_code)
            for item in sorted(
                result.permissions_added,
                key=lambda item: (item.action, item.scope_code),
            )
        ],
        permissions_removed=[
            PermissionResponse(action=item.action, scope_code=item.scope_code)
        ...
        roles_added=[
            ...
            for item in result.roles_added
        ],
        roles_removed=[
            PreviewRoleResponse(
                role_id=item.role_id, role_code=item.role_code, role_name=item.role_name
            )
            for item in result.roles_removed
        ],
    )
